...
func AppendLinkToNode(frptr NodePtr, link Link, toptr NodePtr) {
	...
	case N1GRAM:
		NODE_DIRECTORY.N1directory[frm].I[stindex] = MergeLinkLists(
			NODE_DIRECTORY.N1directory[frm].I[stindex],
			link,
		)
	case N2GRAM:
		NODE_DIRECTORY.N2directory[frm].I[stindex] = MergeLinkLists(
			NODE_DIRECTORY.N2directory[frm].I[stindex],
			link,
		)
	case N3GRAM:
		NODE_DIRECTORY.N3directory[frm].I[stindex] = MergeLinkLists(
			NODE_DIRECTORY.N3directory[frm].I[stindex],
			link,
		)
	case LT128:
		NODE_DIRECTORY.LT128[frm].I[stindex] = MergeLinkLists(
			NODE_DIRECTORY.LT128[frm].I[stindex],
			link,
		)
	case LT1024:
		NODE_DIRECTORY.LT1024[frm].I[stindex] = MergeLinkLists(
			NODE_DIRECTORY.LT1024[frm].I[stindex],
			link,
		)
	case GT1024:
		NODE_DIRECTORY.GT1024[frm].I[stindex] = MergeLinkLists(
			NODE_DIRECTORY.GT1024[frm].I[stindex],
			link,
		)
	}
}
...
func GraphToDB(sst PoSST, wait_counter bool) {
	total := len(
		NODE_DIRECTORY.N1directory,
	) + len(
		NODE_DIRECTORY.N2directory,
	) + len(
		NODE_DIRECTORY.N3directory,
	) + len(
		NODE_DIRECTORY.LT128,
	) + len(
		NODE_DIRECTORY.LT1024,
	) + len(
		NODE_DIRECTORY.GT1024,
	) + len(
		PAGE_MAP,
	)
...
func HubJoin(
	...
	if len(nptrs) != len(weight) {
		fmt.Println(
			"Call to HubJoin with inconsistent node/weight pointer arrays: dimensions ",
			len(nptrs),
			"vs",
			len(weight),
		)
...
func UploadArrowToDB(sst PoSST, arrow ArrowPtr) {
	...
	qstr := fmt.Sprintf(
		"INSERT INTO ArrowDirectory (STAindex,Long,Short,ArrPtr) SELECT %d,'%s','%s',%d WHERE NOT EXISTS (SELECT Long,Short,ArrPtr FROM ArrowDirectory WHERE lower(Long) = lower('%s') OR lower(Short) = lower('%s') OR ArrPtr = %d)",
		staidx,
		long,
		short,
		arrow,
...
func UploadInverseArrowToDB(sst PoSST, arrow ArrowPtr) {
	...
	qstr := fmt.Sprintf(
		"INSERT INTO ArrowInverses (Plus,Minus) SELECT %d,%d WHERE NOT EXISTS (SELECT Plus,Minus FROM ArrowInverses WHERE Plus = %d OR minus = %d)",
		plus,
		minus,
		plus,
		minus,
...
func UploadPageMapEvent(sst PoSST, line PageMap) {
	...
	qstr += fmt.Sprintf(
		"INSERT INTO PageMap (Chap,Alias,Ctx,Line) VALUES ('%s','%s',%d,%d);\n",
		chap,
		line.Alias,
		line.Context,
		line.Line,
...
func AppendDBLinkToNodeCommand(sst PoSST, n1ptr NodePtr, lnk Link, sttype int) string {
	...
	qstr := fmt.Sprintf(
		"UPDATE NODE SET %s=array_append(%s,%s) WHERE (NPtr).CPtr = '%d' AND (NPtr).Chan = '%d' AND (%s IS NULL OR NOT %s = ANY(%s));\n",
		link_table,
		link_table,
		literal,
		n1ptr.CPtr,
...
func AppendDBLinkArrayToNode(sst PoSST, nptr NodePtr, array string, sttype int) string {
	...
	qstr := fmt.Sprintf(
		"UPDATE NODE SET %s='%s' WHERE (NPtr).CPtr = '%d' AND (NPtr).Chan = '%d';\n",
		link_table,
		array,
		nptr.CPtr,
		nptr.Class,
...
func DefineStoredFunctions(sst PoSST) {
	...
	qstr = fmt.Sprintf(
		"CREATE OR REPLACE FUNCTION GetNCCLinks(start NodePtr,exclude NodePtr[],sttype int,chapter text,rm_acc boolean,context text[],maxlimit int)\n" +
			"RETURNS Link[] AS $fn$\n" +
			"DECLARE \n" +
			"    neighbours Link[];\n" +
			"    fwdlinks Link[];\n" +
	...
	for st := -EXPRESS; st <= EXPRESS; st++ {
		...
		qstr += fmt.Sprintf(
			"      FOR this IN SELECT NPtr as thptr,Chap as thchap,%s as chn FROM Node WHERE lower(unaccent(chap)) LIKE lower(chaptxt)\n",
			STTypeDBChannel(st),
		)
		qstr += "      LOOP\n" +
			"         count := 0;\n" +
		...
		qstr += fmt.Sprintf(
			"      FOR this IN SELECT NPtr as thptr,Chap as thchap,%s as chn FROM Node WHERE lower(chap) LIKE lower(chaptxt)\n",
			STTypeDBChannel(st),
		)
		qstr += "      LOOP\n" +
			"         count := 0;\n" +
...
func GetDBNodePtrMatchingNCCS(
	...
	qstr := fmt.Sprintf(
		"SELECT NPtr FROM Node WHERE %s ORDER BY L ASC,(CARDINALITY(Ie3)+CARDINALITY(Im3)+CARDINALITY(Il1)) DESC LIMIT %d",
		NodeWhereString(nm, chap, cn, arrow, seq),
		limit,
	)
...
func GetDBChaptersMatchingName(sst PoSST, src string) []string {
	...
	if remove_accents {
		search := "%" + stripped + "%"
		qstr = fmt.Sprintf(
			"SELECT DISTINCT Chap FROM Node WHERE lower(unaccent(Chap)) LIKE lower('%s')",
			search,
		)
	} else {
		search := "%" + src + "%"
...
func GetDBContextByName(sst PoSST, src string) (string, ContextPtr) {
	...
	if remove_accents {
		search := stripped
		qstr = fmt.Sprintf(
			"SELECT DISTINCT Context,CtxPtr FROM ContextDirectory WHERE unaccent(Context)='%s'",
			search,
		)
	} else {
		search := src
...
func GetDBNodeByNodePtr(sst PoSST, db_nptr NodePtr) Node {
	...
	cols := I_MEXPR + "," + I_MCONT + "," + I_MLEAD + "," + I_NEAR + "," + I_PLEAD + "," + I_PCONT + "," + I_PEXPR
	qstr := fmt.Sprintf(
		"select L,S,Chap,%s from Node where NPtr='(%d,%d)'::NodePtr AND NOT L=0",
		cols,
		db_nptr.Class,
		db_nptr.CPtr,
	)
	...
	if row != nil {
		...
		if count > 1 {
			fmt.Println(
				"GetDBNodeByNodePtr returned too many matches (multi-model conflict?):",
				count,
				"for ptr",
				db_nptr,
			)
...
func GetDBSingletonBySTType(
	...
	for st := 0; st < len(sttypes); st++ {
		...
		if sttypes[st] < 0 {
			fmt.Println(
				"WARNING! Only give positive STType arguments to GetDBSingletonBySTType as both signs are returned as sources (+) and sinks (-)",
			)
			return nil, nil
		}
		...
		qwhere += fmt.Sprintf(
			"(array_length(%s::text[],1) IS NOT NULL AND array_length(%s::text[],1) IS NULL AND match_context((%s)[0].Ctx,%s))",
			stname,
			stinv,
			stname,
			context,
	...
	qstr = fmt.Sprintf(
		"SELECT NPtr FROM Node WHERE lower(Chap) LIKE lower('%s') AND (%s)",
		chapter,
		qwhere,
	)
...
func GetFwdConeAsNodes(sst PoSST, start NodePtr, sttype, depth int, limit int) []NodePtr {
	qstr := fmt.Sprintf(
		"select unnest(fwdconeasnodes) from FwdConeAsNodes('(%d,%d)',%d,%d,%d);",
		start.Class,
		start.CPtr,
		sttype,
		depth,
...
func GetFwdConeAsLinks(sst PoSST, start NodePtr, sttype, depth int) []Link {
	...
	qstr := fmt.Sprintf(
		"select unnest(fwdconeaslinks) from FwdConeAsLinks('(%d,%d)',%d,%d);",
		start.Class,
		start.CPtr,
		sttype,
		depth,
...
func GetFwdPathsAsLinks(sst PoSST, start NodePtr, sttype, depth int, maxlimit int) ([][]Link, int) {
	qstr := fmt.Sprintf(
		"SELECT FwdPathsAsLinks from FwdPathsAsLinks('(%d,%d)',%d,%d,%d);",
		start.Class,
		start.CPtr,
		sttype,
		depth,
...
func GetEntireConePathsAsLinks(
	sst PoSST,
	orientation string,
	start NodePtr,
	depth int,
...
func GetEntireNCConePathsAsLinks(
	sst PoSST,
	orientation string,
	start []NodePtr,
	depth int,
	...
	qstr := fmt.Sprintf(
		"select AllNCPathsAsLinks(%s,'%s',%s,%s,'%s',%d,%d);",
		FormatSQLNodePtrArray(start),
		chapter,
		rm_acc,
		FormatSQLStringArray(context),
		orientation,
		depth,
		limit,
	)
...
func GetConstraintConePathsAsLinks(
	sst PoSST,
	start []NodePtr,
	depth int,
	chapter string,
	...
	qstr := fmt.Sprintf(
		"select ConstraintPathsAsLinks(%s,'%s',%s,%s,%s,%s,%d,%d);",
		nod,
		chapter,
		rm_acc,
		cnt,
...
func MakeCoordinateDirectory(
	XChannels []float64,
	unique [][]NodePtr,
	maxzlen, nth, swimlanes int,
) map[NodePtr]Coords {
	directory := make(map[NodePtr]Coords)
...
func GetPathsAndSymmetries(
	sst PoSST,
	start_set, end_set []NodePtr,
	chapter string,
	context []string,
	arrowptrs []ArrowPtr,
	sttypes []int,
	...
	left_paths, Lnum = GetConstraintConePathsAsLinks(
		sst,
		start_set,
		ldepth,
		chapter,
	...
	right_paths, Rnum = GetConstraintConePathsAsLinks(
		sst,
		end_set,
		rdepth,
		chapter,
	...
	for turn := 0; ldepth < maxdepth && rdepth < maxdepth; turn++ {
		...
		solutions, loop_corrections = WaveFrontsOverlap(
			sst,
			left_paths,
			right_paths,
			Lnum,
		...
		if turn%2 == 0 {
			left_paths = IncConstraintConeLinks(
				sst,
				left_paths,
				chapter,
				context,
	...
	fmt.Println(
		"HINT: specify \\arrow fwd,bwd inverse-pairs to speed restrict search and speed up search",
	)
	return solutions
}
...
func GetConstrainedFwdLinks(
	...
	for _, st := range sttypes {
		...
		qstr := fmt.Sprintf(
			"select GetConstrainedFwdLinks('%s','%s',%s,%s,%s,%d,%s,%d);",
			startnode,
			chapter,
			rm_acc,
			cnt,
...
func GetPathsAndSymmetries_legacy(
	sst PoSST,
	start_set, end_set []NodePtr,
	chapter string,
	context []string,
...
func WaveFrontsOverlap(
	sst PoSST,
	left_paths, right_paths [][]Link,
	Lnum, Rnum, ldepth, rdepth int,
) ([][]Link, [][]Link) {
...
func GetDBAdjacentNodePtrBySTType(
	sst PoSST,
	sttypes []int,
	chap string,
	cn []string,
	...
	for st := 0; st < len(sttypes); st++ {
		...
		stname := STTypeDBChannel(sttypes[st])
		qwhere += fmt.Sprintf(
			"array_length(%s::text[],1) IS NOT NULL AND match_context((%s)[0].Ctx,%s)",
			stname,
			stname,
			context,
		)
	...
	qstr = fmt.Sprintf(
		"SELECT NPtr%s FROM Node WHERE lower(Chap) LIKE lower('%s') AND (%s)",
		qsearch,
		chapter,
		qwhere,
	)
...
func GetNodeOrbit(sst PoSST, nptr NodePtr, exclude_vector string, limit int) [ST_TOP][]Orbit {
	...
	for stindex := 0; stindex < ST_TOP; stindex++ {
		...
		go func(idx int) {
			...
			satellites[idx] = AssembleSatellitesBySTtype(
				sst,
				idx,
				satellites[idx],
				sweep,
...
func AssembleSatellitesBySTtype(
	sst PoSST,
	stindex int,
	satellite []Orbit,
	sweep [][]Link,
...
func GetLastSawSection(sst PoSST) []LastSeen {
	qstr := fmt.Sprintf(
		"SELECT section,nptr,EXTRACT(EPOCH FROM first),EXTRACT(EPOCH FROM last),freq,delta as pdelta,EXTRACT(EPOCH FROM NOW()-last) as ndelta from Lastseen ORDER BY section",
	)
...
func GetLastSawNPtr(sst PoSST, nptr NodePtr) LastSeen {
	...
	qstr := fmt.Sprintf(
		"SELECT section,EXTRACT(EPOCH FROM first),EXTRACT(EPOCH FROM last),freq,delta as pdelta,EXTRACT(EPOCH FROM NOW()-last) as ndelta from Lastseen WHERE NPTR='(%d,%d)'::NodePtr",
		nptr.Class,
		nptr.CPtr,
	)
...
func GetNewlySeenNPtrs(sst PoSST, search SearchParameters) map[NodePtr]bool {
	...
	case RECENT:
		qstr = fmt.Sprintf(
			"SELECT NPtr FROM LastSeen WHERE last > NOW() - INTERVAL '%d hour'",
			search.Horizon,
		)
	case NEVER:
...
func PrintLinkOrbit(satellites [ST_TOP][]Orbit, sttype int, indent_level int) {
	...
	for n := range satellites[t] {
		...
		if satellites[t][n].Ctx != "" {
			txt := fmt.Sprintf(
				" -    (%s) - %s  \t.. in the context of %s\n",
				satellites[t][n].Arrow,
				satellites[t][n].Text,
				satellites[t][n].Ctx,
			)
...
func GetChaptersByChapContext(sst PoSST, chap string, cn []string, limit int) map[string][]string {
	...
	qstr = fmt.Sprintf(
		"SELECT DISTINCT chap,ctx FROM PageMap WHERE match_context(ctx,%s) %s ORDER BY Chap",
		context,
		chap_col,
	)
...
func GetAppointedNodesByArrow(
	sst PoSST,
	arrow ArrowPtr,
	cn []string,
	chap string,
	size int,
) map[ArrowPtr][]Appointment {
	...
	qstr := fmt.Sprintf(
		"SELECT unnest(GetAppointments(%d,%d,%d,'%s',%s,%v))",
		int(reverse_arrow),
		sttype,
		size,
		chap_col,
...
func GetAppointedNodesBySTType(
	sst PoSST,
	sttype int,
	cn []string,
	chap string,
...
func Fractionate(
	frag string,
	L int,
	frequency [N_GRAM_MAX]map[string]float64,
	min int,
) [N_GRAM_MAX][]string {
...
func AssessStaticTextAnomalies(
	L int,
	frequencies [N_GRAM_MAX]map[string]float64,
	locations [N_GRAM_MAX]map[string][]int,
) ([N_GRAM_MAX][]TextRank, [N_GRAM_MAX][]TextRank) {
...
func AssessTextCoherentCoactivation(
	L int,
	ngram_loc [N_GRAM_MAX]map[string][]int,
) ([N_GRAM_MAX]map[string]int, [N_GRAM_MAX]map[string]int, int) {
...
func CoherenceSet(
	ngram_loc [N_GRAM_MAX]map[string][]int,
	L, coherence_length int,
) ([N_GRAM_MAX][]map[string]int, int) {
...
func NextWord(
	frag string,
	rrbuffer [N_GRAM_MAX][]string,
) ([N_GRAM_MAX][]string, [N_GRAM_MAX][]string) {
