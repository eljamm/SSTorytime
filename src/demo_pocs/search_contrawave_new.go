...
func main() {
	...
	solutions := GetPathsAndSymmetries2(
		sst,
		leftptrs,
		rightptrs,
		chapter,
...
func GetPathsAndSymmetries2(
	sst SST.PoSST,
	start_set, end_set []SST.NodePtr,
	chapter string,
	context []string,
	arrowptrs []SST.ArrowPtr,
	sttypes []int,
	...
	left_paths, Lnum = SST.GetConstraintConePathsAsLinks(
		sst,
		start_set,
		ldepth,
		chapter,
	...
	right_paths, Rnum = SST.GetConstraintConePathsAsLinks(
		sst,
		end_set,
		rdepth,
		chapter,
	...
	for turn := 0; ldepth < maxdepth && rdepth < maxdepth; turn++ {
		...
		solutions, loop_corrections = SST.WaveFrontsOverlap(
			sst,
			left_paths,
			right_paths,
			Lnum,
...
func IncConstraintConeLinks(
	...
	for p := 0; p < len(cone); p++ {
		...
		branch := cone[p]
		exclude := make(map[SST.NodePtr]bool)
