...
func main() {
	...
	for n := 1; n < SST.N_GRAM_MAX; n++ {
		...
		sort.Slice(amb, func(i, j int) bool {
			return SST.StaticIntentionality(
				L,
				amb[i],
				SST.STM_NGRAM_FREQ[n][amb[i]],
			) > SST.StaticIntentionality(
				L,
				amb[j],
				SST.STM_NGRAM_FREQ[n][amb[j]],
			)
		})
		sort.Slice(cond, func(i, j int) bool {
			return SST.StaticIntentionality(
				L,
				cond[i],
				SST.STM_NGRAM_FREQ[n][cond[i]],
			) > SST.StaticIntentionality(
				L,
				cond[j],
				SST.STM_NGRAM_FREQ[n][cond[j]],
			)
		})
		...
		for i := 0; i < 150 && i < len(amb); i++ {
			fmt.Println(
				...
				amb[i],
				"       ",
				SST.StaticIntentionality(L, amb[i], SST.STM_NGRAM_FREQ[n][amb[i]]),
			)
		}
		...
		for i := 0; i < 150 && i < len(cond); i++ {
			fmt.Println(
				...
				cond[i],
				"       ",
				SST.StaticIntentionality(L, cond[i], SST.STM_NGRAM_FREQ[n][cond[i]]),
			)
		}
	}
}
