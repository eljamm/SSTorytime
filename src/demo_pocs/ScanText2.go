...
var (
	EXCLUSIONS     []string
	LEG_WINDOW     int = 100 // sentences per leg
	STM_NGRAM_RANK [N_GRAM_MAX]map[string]float64
)
	...
	for input := 0; input < len(args); input++ {
		...
		for p := range pbs {
			for s := range pbs[p] {
				...
				score := FractionateThenRankSentence(
					s,
					pbs[p][s],
					scale,
					ltm_every_ngram_occurrence,
				)
...
func FractionateThenRankSentence(
	...
	sentence string,
	scale float64,
	ltm_every_ngram_occurrence [N_GRAM_MAX]map[string][]int,
) float64 {
	...
	for f := range sentence_frags {
		...
		for word := range clean_sentence {
			...
			rank, rrbuffer = NextWordAndUpdateLTMNgrams(
				s_idx,
				cleanword,
				rrbuffer,
				scale,
...
func CheckArrow(alias, name string) {
	...
	if ok {
		ParseError(
			ERR_ARR_REDEFINITION + "\"" + alias + "\" previous short name: " + SST.ARROW_DIRECTORY[prev].Short,
		)
		os.Exit(-1)
	}
	...
	if ok {
		ParseError(
			ERR_ARR_REDEFINITION + "\"" + name + "\" previous long name: " + SST.ARROW_DIRECTORY[prev].Long,
		)
		os.Exit(-1)
	}
}
