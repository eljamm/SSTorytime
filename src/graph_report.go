...
func Usage() {
	fmt.Printf(
		"usage: graph_report [-sttype comma separated L,C,P,N] [-depth integer] [-chapter comma separated string] [context]\n",
	)
	flag.PrintDefaults()
...
func AnalyzeGraph(sst SST.PoSST, chapter string, context []string, sttypes []int, depth int) {
	...
	for class := 1; class < 7; class++ {
		if distribution[class] > 0 {
			fmt.Printf(
				"  - %s : %d / %d\n",
				SST.CLASS_CHANNEL_DESCRIPTION[class],
				distribution[class],
				total,
			)
		}
...
func AnalyzePowerMatrix(sst SST.PoSST, symbolic [][]string) (map[string]int, map[string][]int) {
	...
	memberlist := make(map[string][]int)
