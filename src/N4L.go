...
var (
	LINE_NUM        int = 1
	LINE_ITEM_CACHE     = make(
		map[string][]string,
	) // contains current and labelled line elements
...
func ArrowCollision(arr SST.ArrowPtr, short, long string) {
	if arr < 0 {
		ParseError(
			ERR_ARR_REDEFINITION + "long \"" + long + "\"/" + "short \"" + short + "\" seems to be previously used somewhere",
		)
		// os.Exit(-1)
	}
}
...
func CompleteCloseness(sst SST.PoSST, node SST.Node) {
	equivalences := make(map[SST.ArrowPtr]int)
...
func SummarizeGraph() {
	...
	fmt.Println(
		"Total links",
		total,
		"sparseness (fraction of completeness)",
		float32(total)/float32(complete),
	)
}
...
func CreateAdjacencyMatrix(searchlist string) (int, []SST.NodePtr, [][]float32, [][]float32) {
	...
	for f := 0; f < len(filtered_node_list); f++ {
		Verbose(
			...
			dim,
			"]",
			SST.GetNodeTxtFromPtr(filtered_node_list[f]),
		)
	}
	...
	for class := SST.N1GRAM; class <= SST.GT1024; class++ {
		...
		case SST.N1GRAM:
			for n := range SST.NODE_DIRECTORY.N1directory {
				node_list = SearchIncidentRowClass(
					SST.NODE_DIRECTORY.N1directory[n],
					search_list,
					node_list,
					weights,
				)
		...
		case SST.N2GRAM:
			for n := range SST.NODE_DIRECTORY.N2directory {
				node_list = SearchIncidentRowClass(
					SST.NODE_DIRECTORY.N2directory[n],
					search_list,
					node_list,
					weights,
				)
		...
		case SST.N3GRAM:
			for n := range SST.NODE_DIRECTORY.N3directory {
				node_list = SearchIncidentRowClass(
					SST.NODE_DIRECTORY.N3directory[n],
					search_list,
					node_list,
					weights,
				)
...
func SearchIncidentRowClass(
	node SST.Node,
	searcharrows []SST.ArrowPtr,
	node_list []SST.NodePtr,
	ret_weights map[RCtype]float32,
) []SST.NodePtr {
...
func AddMandatory() {
	...
	inv = SST.InsertArrowDirectory(
		"contains",
		SST.INV_CONT_FOUND_IN_S,
		SST.INV_CONT_FOUND_IN_L,
		"-",
	)
	SST.InsertInverseArrowDirectory(arr, inv)
	...
	inv = SST.InsertArrowDirectory(
		"properties",
		SST.INV_EXPR_AMBIENT_S,
		SST.INV_EXPR_AMBIENT_L,
		"-",
	)
	SST.InsertInverseArrowDirectory(arr, inv)
...
func IdempAddLink(from string, frptr SST.NodePtr, link SST.Link, to string, toptr SST.NodePtr) {
	...
	if link.Wgt != 1 {
		PVerbose(
			...
			from,
			"--(",
			SST.ARROW_DIRECTORY[link.Arr].Long,
			",",
			link.Wgt,
			")->",
			to,
			SST.CONTEXT_DIRECTORY[link.Ctx],
		)
	} else {
...
func ReadToLast(src []rune, pos int, stop rune) (string, int) {
	...
	if IsQuote(stop) && src[pos-1] != stop {
		e := fmt.Sprintf(
			"%s starting at line %d (found token %s)",
			ERR_MISMATCH_QUOTE,
			starting_at,
			string(cpy),
		)
...
func IsGeneralString(src []rune, pos int) bool {
	...
	case ')':
		...
		msg := fmt.Sprintf(
			"%s at position %d near '...%s...'",
			ERR_STRAY_PAREN,
			pos,
			string(src[before:after]),
		)
		ParseError(msg)
		os.Exit(-1)
	case '(':
...
func PrintLink(l SST.Link) {
	...
	Verbose(
		...
		l.Ctx,
		" \t . . .",
		SST.PrintSTAIndex(arrow.STAindex),
	)
}
